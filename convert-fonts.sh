...
echo "add ttf or woff and woff2 files in current directory"

# check if has ttf or woff files in current directory
if [[ ! $(find . -name "*.woff" || -name "*.woff2") ]]; then
    echo "No TTF, WOFF, or WOFF2 files found in the directory."
    exit 1
fi
